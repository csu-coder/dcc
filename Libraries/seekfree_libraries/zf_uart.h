...
typedef enum{
	EMPTY,                //空
	RECEIVING,	 		  //正在接收
	RECEIVING_COMPLETED,  //接收完成
	REFUSE_TO_RECEIVE,    //拒绝接收
...
extern uint8 USART4_RX_STA;       //接收状态标记
extern uint16 USART4_RX_LEN;   					//接收长度
extern uint8 uart4_tx_buff[UART4_TX_BUFFER_SIZE];	//发送缓冲
extern uint8 uart4_rx_buff[UART4_RX_BUFFER_SIZE];	//接收缓冲
extern uint8 busy[5];
